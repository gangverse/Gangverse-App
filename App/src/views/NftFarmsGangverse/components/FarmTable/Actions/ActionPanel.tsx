...
import { getNftFarmGangverseBlockInfo } from 'views/NftFarmsGangverse/helpers'
...
export interface ActionPanelProps {
  ...
  details: NftFarmGangverseWithStakedValue
...
const ActionPanel: React.FunctionComponent<ActionPanelProps> = ({
  ...
  const bsc = getCronosScanLink(nftAddress, 'address')
  const farmConfig = nftFarmsGangverseConfig.filter((farmConfig) => farmConfig.pid == farm.pid)[0]

  const { shouldShowBlockCountdown, blocksUntilStart, blocksRemaining, hasPoolStarted, blocksToDisplay } =
    getNftFarmGangverseBlockInfo(farm.startBlock, farm.endBlock, farm.isFinished, currentBlock)
        ...
        {isActive && (
          <StakeContainer>
            <StyledLinkExternal href={apyModalLink}>
              {t('Get %symbol%', { symbol: lpLabel.replace('LINBING', '') })}
            </StyledLinkExternal>
          </StakeContainer>
        )}
