...
import { useLinbingNFTContract, useGangverseDarksideNFTContract, useERC20, useIfoV2Contract } from 'hooks/useContract'
import { multicallCronosv1, multicallv2 } from 'utils/multicall'
import ifoV2Abi from 'config/abi/ifoV2.json'
import ifoV3Abi from 'config/abi/ifoV3.json'
import linbingAbi from 'config/abi/linbingNft.json'
...
const useGetWalletIfoData = (ifo: Minting): WalletIfoData => {
  ...
  const contract = useGangverseDarksideNFTContract(address)
  ...
  const fetchIfoData = useCallback(async () => {
    ...
    const abi = GangverseDarksideNFTAbi

    //const [userInfo, amounts, isQualifiedNFT, isQualifiedPoints] = await multicallv2(abi, [...ifoCalls, ...ifov3Calls])
    const [isHolder, discountAmount] = await multicallCronosv1(abi, [...mintingV3Calls])
