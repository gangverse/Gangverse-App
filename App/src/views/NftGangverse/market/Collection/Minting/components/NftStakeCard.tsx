...
import { useFarmFromPid, usePriceCakeBusd } from 'state/nftFarmsGangverse/hooks'
import { useIfoPoolCredit, useIfoPoolVault, useIfoWithApr } from 'state/pools/hooks'
import styled from 'styled-components'
import { getBalanceNumber } from 'utils/formatBalance'
import CardActionsContainer from 'views/NftFarmsGangverse/components/FarmCard/CardActionsContainer'
import CardHeading from 'views/NftFarmsGangverse/components/FarmCard/CardHeading'
...
import DetailsSection from 'views/NftFarmsGangverse/components/FarmCard/DetailsSection'
...
const NftStakeCardBody = ({ farm, account }) => {
  ...
  const lpLabel = farm.lpSymbol && farm.lpSymbol.replace('Linbing', '')
  const earnLabel = farm.earningToken ? farm.earningToken.symbol: ('LINBING')
  const apyModalLink = "/nfts/collections"
  const nftAddress = getAddress(farm.nftAddresses)
  const isPromotedFarm = true //farm.token.symbol === 'LINBING' Caution: Fix
  ...
  const projectLink = nftFarmsGangverseConfig.filter((f) => f.pid == farm.pid)[0]["projectLink"]
  ...
  const { cakeRewardsApr, lpRewardsApr } = getNftFarmGangverseApr(new BigNumber(farm.poolWeight), farm.tokenPerBlock ? parseFloat(farm.tokenPerBlock) : null, totalLiquidityWithThreshold, mainCollectionWeight)
              ...
              {sideRewards.length === 0 ? (
                ...
                <>
                  <Flex justifyContent="center" alignItems="center">
                    <Text color='#fdda5a'>{'DAILY REWARDS'}</Text>
                  </Flex>
