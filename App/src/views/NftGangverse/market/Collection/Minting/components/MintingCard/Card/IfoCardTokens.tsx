...
import { PublicIfoData, WalletIfoData } from 'views/NftGangverse/market/Collection/Minting/types'
...
import NFTMedia from 'views/NftGangverse/market/components/NFTMedia'
import PreviewImage from 'views/NftGangverse/market/components/CollectibleCard/PreviewImage'
      ...
      return (
        <>
          ...
          {(account && !isHolder && version == 3.1) && (
            ...
            <Box>
            <MessageText display="inline" color='#ffffff'>
            {`You don't have any Cronos Cats NFT, NFT holders get %${holderDiscountPercentage} discount and doesn't have to wait for the countdown.`}
              </MessageText>{' '}
              <MessageTextLink display="inline" fontWeight={700} href="https://cronoscats.club" target="_blank" color="failure">
                {t('Not too late')} »
              </MessageTextLink>
            </Box>
          </Message>
          ...
          {(account && isHolder && version == 3.1) && (
            <Message mt="24px" p="8px" variant="success">
              <MessageText small display="inline">
                {t(`Wow! You are holder. You save ${discountAmount} CRO and no need to wait for the countdown.`)}
              </MessageText>
            </Message>
          )}
