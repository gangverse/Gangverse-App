...
import { useGangverseDarksideNFTContract } from "hooks/useContract"
...
import { walletOfOwnerApi } from 'state/nftMarketGangverse/helpers'


export const useNftsForCollectionAndAddress = (selectedPid: number) => {
    const { account } = useWeb3React()
    
    const nftPool = nftFarmsGangverseConfig.filter(({ pid }) => pid == selectedPid)[0]
    const collectionAddress = getAddress(nftPool.nftAddresses)
    const collectionContract = useGangverseDarksideNFTContract(collectionAddress)


    const getNfts = async () => {
      try {
        const tokenIds = nftPool["useApi"] ? await walletOfOwnerApi(account, collectionAddress) : await collectionContract.tokensOfOwner(account);
