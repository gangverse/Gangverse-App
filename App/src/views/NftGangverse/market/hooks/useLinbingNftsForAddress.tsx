...
import { useGetCollections } from 'state/nftMarketGangverse/hooks'
import { ApiCollections, ApiResponseSpecificToken, NftLocation, NftToken, TokenIdWithCollectionAddress } from 'state/nftMarketGangverse/types'
...
import { attachMarketDataToWalletNfts, combineNftMarketAndMetadata, getNftsMarketData } from 'state/nftMarketGangverse/helpers'
...
export const fetchWalletTokenIdsForCollections = async (
  ...
  const balanceOfCallsResultRaw = await multicallCronosv1(erc721Abi, balanceOfCalls)
  ...
  try {
    tokenIdResultRaw = await multicallCronosv1(GangverseDarksideNFTAbi, tokenIdCalls)
  } catch (error) {
    console.log(error)
  }
  // TODO: Deploy multicallv2 and activate instead of v1
...
export const getNftsFromDifferentCollectionsApi = async (
  ...
  const rawTokenURIs = await multicallCronosv1(erc721ABI, imageCalls)
