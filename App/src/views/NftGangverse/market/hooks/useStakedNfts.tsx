...
import { getLinbingNftStakeAddress } from 'utils/addressHelpers'
import { useFarmUser } from 'state/nftFarmsGangverse/hooks'
...
export const useStakedNfts = (selectedPid: number) => {
  ...
  const nftStakeContractAddress = getLinbingNftStakeAddress()
  ...
  const getNfts = async () => {
    try {
      ...
      const rawTokens = await multicallCronosv1(smartNftStakeAddress ? smartNftStakeABI : linbingNftStakeABI, calls)
