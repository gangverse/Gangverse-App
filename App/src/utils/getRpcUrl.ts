...
// Array of available nodes to connect to
export const cronosNodes = [process.env.NEXT_PUBLIC_CRONOS_NODE_1]
...
export const getCronosNodeUrl = () => {
  // Use custom node if available (both for development and production)
  // However on the testnet it wouldn't work, so if on testnet - comment out the NEXT_PUBLIC_NODE_PRODUCTION from env file
  if (process.env.NEXT_PUBLIC_CRONOS_NODE_PRODUCTION) {
    return process.env.NEXT_PUBLIC_CRONOS_NODE_PRODUCTION
  }

  if (process.env.NEXT_PUBLIC_CHAIN_ID == "25") {
    return sample(cronosNodes)
  }
