import { ChainId, Pair, Token } from '@linbingdefi/sdk'
...
import { AppDispatch, AppState } from '../../index2'
import {
  addSerializedPair,
  addSerializedToken,
  FarmStakedOnly,
...
export function useGasPrice(chainIdOverride?: number): string {
  ...
  const { data: cronosProviderGasPrice = GAS_PRICE_GWEI.default } = useSWR(
    library &&
      library.provider &&
      chainId === ChainId.CRONOS &&
      userGas === GAS_PRICE_GWEI.rpcDefault && ['cronosProviderGasPrice', library.provider],
    async () => {
  ...
  if (chainId === ChainId.CRONOS) {
    return userGas === GAS_PRICE_GWEI.rpcDefault ? cronosProviderGasPrice : userGas
  }
...
export function toV2LiquidityToken([tokenA, tokenB]: [Token, Token]): Token {
  return new Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'Linbing-LP', 'Linbing LPs')
}
