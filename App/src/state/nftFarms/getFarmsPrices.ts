...
const getFarmsPrices = (farms: SerializedFarm[]) => {
  ...
  // CAUTION: This codes added by Linbing to calculate cro/usdt price
  // If we have cro/usdt pair directly, we wont't need this anymore.
  const collectCroFarm = farms.find((farm) => farm.pid === 2)
  const collectPriceCro = collectCroFarm.tokenPriceVsQuote ? BIG_ONE.div(collectCroFarm.tokenPriceVsQuote) : BIG_ZERO
  const croPriceUsdt = collectPriceCro.div(collectPriceUsdt)

  const farmsWithPrices = farms.map((farm) => {
    const quoteTokenFarm = getFarmFromTokenSymbol(farms, farm.quoteToken.symbol)
    const tokenPriceBusd = getFarmBaseTokenPrice(farm, quoteTokenFarm, croPriceUsdt)
    const quoteTokenPriceBusd = getFarmQuoteTokenPrice(farm, quoteTokenFarm, croPriceUsdt)
