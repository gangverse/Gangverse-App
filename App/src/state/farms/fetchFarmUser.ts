...
import linbingFarmABI from 'config/abi/linbingFarm.json'
import { multicallCronosv1 } from 'utils/multicall'
import { getAddress, getLinbingFarmAddress } from 'utils/addressHelpers'
...
export const fetchFarmUserAllowances = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  ...
  const rawLpAllowances = await multicallCronosv1<BigNumber[]>(erc20ABI, calls)
...
export const fetchFarmUserTokenBalances = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  ...
  const rawTokenBalances = await multicallCronosv1(erc20ABI, calls)
...
export const fetchFarmUserStakedBalances = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  ...
  const rawStakedBalances = await multicallCronosv1(masterchefABI, calls)
...
export const fetchFarmUserEarnings = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  ...
  const rawEarnings = await multicallCronosv1(linbingFarmABI, calls)
