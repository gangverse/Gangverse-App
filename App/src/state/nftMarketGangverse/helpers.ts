 ...
 */
 export const walletOfOwnerApi = async (
  ...
  const baseUrl = `https://cronos-${network}.g.alchemy.com/nft/v2/96Zf7eDc5vF4TiNAnT2ibM-S5Fj4vmLp/getNFTs`;
...
const fetchCollectionsTotalSupply = async (collections: ApiCollection[]): Promise<number[]> => {
  ...
  if (totalSupplyCalls.length > 0) {
    const totalSupplyRaw = await multicallCronosv1(erc721Abi, totalSupplyCalls)
