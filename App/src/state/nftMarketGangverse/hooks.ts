...
import { pancakeBunniesAddress } from 'views/NftGangverse/market/constants'
...
export const useGetCollections = (): { data: ApiCollections; status: FetchStatus } => {
  const { data, status } = useSWR(['nftMarketGangverse', 'collections'], async () => getCollections())
...
export const useGetCollection = (collectionAddress: string): Collection | undefined => {
  ...
  const { data } = useSWR(
    checksummedCollectionAddress ? ['nftMarketGangverse', 'collections', checksummedCollectionAddress.toLowerCase()] : null,
...
export const useGetShuffledCollections = (): { data: ApiCollections; status: FetchStatus } => {
  const { data } = useSWRImmutable(['nftMarketGangverse', 'collections'], async () => getCollections())
  const collections = data ?? ({} as ApiCollections)
  const { data: shuffledCollections = {}, status } = useSWRImmutable(
    !isEmpty(collections) ? ['nftMarketGangverse', 'shuffledCollections'] : null,
    () => {
