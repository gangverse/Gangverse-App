...
import { getAddress, getLinbingPoolAddress } from 'utils/addressHelpers'
import { getPoolApr } from 'utils/apr'
import { BIG_ZERO } from 'utils/bigNumber'
import { getLinbingContract } from 'utils/contractHelpers'
...
const cakeContract = getLinbingContract()
...
export const fetchCakePoolUserDataAsync = (account: string) => async (dispatch) => {
  ...
  const [[allowance], [stakingTokenBalance]] = await multicallCronosv2(cakeAbi, cakeContractCalls)

  const masterChefCalls = ['pendingReward', 'userInfo'].map((method) => ({
    address: getLinbingPoolAddress(),
    name: method,
    params: ['0', account],
  }))
  const [[pendingReward], { amount: masterPoolAmount }] = await multicallCronosv2(linbingPool, masterChefCalls)
...
export const fetchPoolsPublicDataAsync = (currentBlockNumber: number) => async (dispatch, getState) => {
  try {
    ...
    if (!currentBlock) {
      currentBlock = await simpleCronosRpcProvider.getBlockNumber()
    }
