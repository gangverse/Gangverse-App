...
import fetchFarms, { fetchNftFarmsGangverseBlockLimits, fetchNftPoolsStakingLimits } from './fetchFarms'
...
import { SerializedNftFarmsGangverseState, SerializedNftFarmGangverse } from '../typesgangverse'
...
const initialState: SerializedNftFarmsGangverseState = {
...
export const fetchFarmsPublicDataAsync = createAsyncThunk<
  [SerializedNftFarmGangverse[], number],
  {pids: number[]; currentBlock: number},
  {
    state: AppState
  }
>(
  'nftFarmsGangverse/fetchFarmsPublicDataAsync',
  async ({pids, currentBlock}) => {
    ...
    const farmsWithoutHelperLps = farms.filter((farm: SerializedNftFarmGangverse) => {   // farmsWithPrices --> farms
  ...
  },
  {
    condition: (arg, { getState }) => {
      const { nftFarmsGangverse } = getState()
      if (nftFarmsGangverse.loadingKeys[stringify({ type: fetchFarmsPublicDataAsync.typePrefix, arg })]) {
...
interface NftFarmGangverseUserDataResponse {
  pid: number
  allowance: boolean[]
  tokenBalance: string
...
export const fetchFarmUserDataAsync = createAsyncThunk<
  NftFarmGangverseUserDataResponse[],
  { account: string; pids: number[] },
  {
    state: AppState
  }
>(
  'nftFarmsGangverse/fetchFarmUserDataAsync',
  async ({ account, pids }) => {
    const farmsToFetch = farmsConfig.filter((farmConfig) => pids.includes(farmConfig.pid))
...
export const fetchNftPoolsStakingLimitsAsync = () => async (dispatch, getState) => {
  const poolsWithStakingLimit = getState()
    .nftFarmsGangverse.data.filter(({ stakingLimit }) => stakingLimit !== null && stakingLimit !== undefined)
    .map((pool) => pool.pid)
