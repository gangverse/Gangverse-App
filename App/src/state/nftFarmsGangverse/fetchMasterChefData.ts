...
const masterChefFarmCalls = (farm: SerializedNftFarmGangverse) => {
  const { pid } = farm
  return pid || pid === 0
    ? [
        {
...
const smartNftPoolCalls = (farm: SerializedNftFarmGangverse) => {
...
export const fetchMasterChefData = async (farms: SerializedNftFarmGangverseConfig[], isSmartNftPool: boolean): Promise<any[]> => {
