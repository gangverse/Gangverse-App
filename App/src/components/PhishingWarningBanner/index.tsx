...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://linbing.biz - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/linbing.biz)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://linbing.biz'}
          color={text === 'https://linbing.biz' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
