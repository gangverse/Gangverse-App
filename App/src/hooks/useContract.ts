...
import {
  ...
  getLinbingNFTContract,
  getGangverseDarksideNFTContract,
  getLinbingPoolContract,
  getLinbingAutoPoolVaultContract,
  getLinbingContract,
  getLinbingFarmContract,
  getLinbingNftStakeContract,
  getLinbingClaimRewardContract,
  getLinbingClaimRewardV2Contract,
  getSmartNftStakeContract,
  getSmartNftStakeGangverseContract,
} from 'utils/contractHelpers'
import { getMulticallAddress, getMulticallCronosAddress } from 'utils/addressHelpers'
import { VaultKey } from 'state/types'
import {
  CakeVault,
  LinbingAutoPoolVault,
  EnsPublicResolver,
  EnsRegistrar,
  Erc20,
  Erc20Bytes32,
...
import { ChainId, WETH } from '@linbingdefi/sdk'
...
export const useLinbingNFTContract = (address: string) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getLinbingNFTContract(address, library.getSigner()), [library])
}

export const useGangverseDarksideNFTContract = (address: string) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getGangverseDarksideNFTContract(address, library.getSigner()), [library])
}
...
export const useLinbing = (withSignerIfPossible = true) => {
  const { account, library } = useActiveWeb3React()
  return useMemo(
    () => getLinbingContract(withSignerIfPossible ? getProviderOrSigner(library, account) : null),
    [account, library, withSignerIfPossible],
  )
}

export const useLinbingClaimRewardContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getLinbingClaimRewardContract(library.getSigner()), [library])
}

export const useLinbingClaimRewardV2Contract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getLinbingClaimRewardV2Contract(library.getSigner()), [library])
}
...
export const useLinbingPool = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getLinbingPoolContract(library.getSigner()), [library])
}

// Only Farm Version Masterchef
export const useLinbingFarm = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getLinbingFarmContract(library.getSigner()), [library])
}

// Nft Farm Version Masterchef
export const useLinbingNftStake = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getLinbingNftStakeContract(library.getSigner()), [library])
}
...
export const useSmartNftStakeGangverseContract = (id) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getSmartNftStakeGangverseContract(id, library.getSigner()), [id, library])
}
...
export const useVaultPoolContract = (vaultKey: VaultKey): LinbingAutoPoolVault | IfoPool => {
  const { library } = useActiveWeb3React()
  return useMemo(() => {
    return vaultKey === VaultKey.CakeVault
      ? getLinbingAutoPoolVaultContract(library.getSigner())
...
// New
export const useLinbingAutoPoolVaultContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getLinbingAutoPoolVaultContract(library.getSigner()), [library])
}
...
export function useMulticallContract() {
  return useContract<Multicall>(getMulticallCronosAddress(), multiCallAbi, false)
}
